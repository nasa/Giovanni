//$Id: Registry.js,v 1.3 2013/05/23 15:07:05 rstrub Exp $ 
//-@@@ Giovanni, Version $Name:  $

// establish the component namespace
giovanni.namespace("util");
// component constructor
giovanni.util.Registry = function () {
  ...
  this.fire = function fire(comp,noFireVal){
	...
	//var consumers = new Array();
	  ...
	  //consumers = this.getConsumers( evtObjA[i].name );
  ...
  this.syncComponents = function() {
    ...
    //giovanni.util.uniqueArray(producers);
...
*/
  ...
  // get all of the user interface (selection) components in compRegistry 
  this.getUIClassComponents = function getUIClassComponents(className) {
    var rawA = this.getComponents();
    var coA = new Array();
    for(var i=0;i<rawA.length;i++){
      if(rawA[i].obj.loadFromQuery){
	if(rawA[i].obj.classId&&rawA[i].obj.classId==className){
          coA.push(rawA[i].obj);
	}
      }
    }
...
var REGISTRY = new giovanni.util.Registry();
