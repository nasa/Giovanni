...
my $program = 'g4_ex_area_avg_diff_time.pl';
...
exit(1) unless is( $diff_rc, '', "Outcome of diff" );
...
<units>
    <linearConversions>
        <linearUnit source="mm/day" destination="mm/hr" scale_factor="1.0/24.0"
            add_offset="0" />
        <linearUnit source="K" destination="C"
            scale_factor="1.0" add_offset="-273.15" />
    </linearConversions>
...
variables:
    int time(time) ;
        ...
        time:units = "seconds since 1970-01-01 00:00:00" ;
    float val(time) ;
        val:_FillValue = -9999.f ;
        val:Serializable = "True" ;
        val:coordinates = "time" ;
        val:long_name = "Atmospheric Temperature Profile, 1000 to 1 hPa, daytime (ascending), AIRS, 1 x 1 deg." ;
        val:missing_value = -9999.f ;
        val:product_short_name = "AIRX3STD" ;
        val:product_version = "006" ;
        val:quantity_type = "Air Temperature" ;
        val:standard_name = "air_temperature" ;
        val:units = "C" ;
        val:cell_methods = "TempPrsLvls_A: mean lat, lon: mean" ;
    int dataday(time) ;
        dataday:standard_name = "Standardized Date Label" ;
...
data:
 ...
 val = -0.9884639, -0.863161, -0.1213654 ;

 dataday = 2009017, 2009018, 2009019 ;

//Lat    cos(lat)      Ascending             Descending              Differences         Weighted Diff	
//47.5   0.675590208   275.1875   275.5000   276.6875   276.8125   -1.5    -1.3125   -1.013385311 -0.886712147
//48.5   0.662620048   275.2500   275.6875   276.6875   275.3750   -1.4375  0.3125   -0.952516319  0.207068765
//SUM(I2:J3)/(2*(B2+B3))									-0.988463884
}
