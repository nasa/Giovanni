...
giovanni_wrapper.pl - a wrapper calling scientist-provided algorithms

=head1 SYNOPSIS

giovanni_wrapper.pl --program 'text' 
...
giovanni_time_averager.pl simplifies and regularizes the calling arguments for
scientist provided algorithims. 
=over
...
Comma separated list of input XML G4 Manifest files. 
...
Whether the output data has a time axis. This is true for Hovmollers and time-axis.
If unset, assumes no time axis.
This is what controls whether Giovanni::Visualizer::TimeTics is invoked.
...
# $Id: giovanni_wrapper.pl,v 1.16 2015/04/16 22:15:42 clynnes Exp $
# -@@@ Giovanni-4
...
my $outnc = Giovanni::Algorithm::Wrapper::run(%args);
exit( !( -s $outnc ) );
