...
cleanCacheDb - cleans cache files to remove key entries whose corresponding 
files have disappeared

=head1 SYNOPSIS

  perl cleanCacheDb.pl /path/to/cache [region_1 region_2 region_3 ...]

=head1 DESCRIPTION

Removed cache key entries for files that are no longer in the cache. Prints out
which entries were removed. If you don't specify any regions, the code will 
clean all regions.
