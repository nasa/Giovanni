...
# Temporary files
my $ncfiles = [
    "test_AeraAveragerNco1.nc", "test_AeraAveragerNco2.nc",
    "test_AeraAveragerNco3.nc"
];
my $outfile = "test_AreaAveragerNco.aavg.nc";

my $infile = "test_AreaAveragerNco.txt";
`echo "test_AeraAveragerNco1.nc" > $infile`;
`echo "test_AeraAveragerNco2.nc" >> $infile`;
`echo "test_AeraAveragerNco3.nc" >> $infile`;

# Create test data
my $rc = create_test_data($ncfiles);

ok( ( -f $ncfiles->[0] ), "Created test nc file 1/3" ) || $err++;
ok( ( -f $ncfiles->[1] ), "Created test nc file 2/3" ) || $err++;
ok( ( -f $ncfiles->[2] ), "Created test nc file 3/3" ) || $err++;

# Test area_averager method
my $dims = [ "lat", "lon" ];
my $miss = "-1.267651e+30";
my $vars = ["FinalAerosolAbsOpticalDepth500"];
my $bbox = "71.5,-0.5,88.5,8.5";

my $rc1
    = Giovanni::Algorithm::AreaAveragerNco::area_averager( $infile, $outfile,
    $dims, $vars, undef, $miss, $bbox );

ok( ( -f $outfile ), "Results file created" ) || $err++;

# Test results
my $output = `ncdump -v FinalAerosolAbsOpticalDepth500 $outfile`;
my ($result) = ( $output =~ /FinalAerosolAbsOpticalDepth500 = (.*);/gs );
$result =~ s/\s//g;
ok( $result eq "0.00993804095796851,0.0132276414163548,0.0042730141320149",
