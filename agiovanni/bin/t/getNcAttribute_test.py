...
"""
NAME
t_getNcAttribute.py - Unit test script of getNcAttribute.py

AUTHORS
Jianfu Pan, 04/27/2012, Initial version
"""

import os
import unittest

class testGetNcAttribute(unittest.TestCase):
    def setUp(self):
        self.cmd = "getNcAttribute.py"
        self.ncfile = "t_NcFile.nc"

    def testNcfile(self):
        self.assertTrue(os.path.exists(self.ncfile), "Missing t_NcFile.nc")

    def testVariableAttribute(self):
        # Testing variable's title attribute
        att = "title"
        var = "h5__HDFEOS_GRIDS_Aerosol_NearUV_Grid_Data_Fields_FinalAerosolAbsOpticalDepth388"
        cmd = self.cmd + " -a " + att + " -v " + var + " " + self.ncfile
        output = os.popen(cmd).readlines()[0].rstrip()
        self.assertEqual(output, "Final Aerosol Absorption Optical Depth at 388 nm", "Incorrect attribute")

    def testGlobalAttribute(self):
        # Testing global title attribute
        att = "title"
        cmd = self.cmd + " -a " + att + " " + self.ncfile
        output = os.popen(cmd).readlines()[0].rstrip()
        self.assertEqual(output, "NASA HDFEOS5 Grid", "Incorrect global attribute")
...
suite = unittest.TestLoader().loadTestsFromTestCase(testGetNcAttribute)
