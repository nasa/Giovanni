...
my $rc = Giovanni::PostProcess::run(
    ...
    'endtime'     => '2003-01-01T23:59',
    'infile'      => $mfstInput,
    'name'        => 'Time Averaged Map',
    'outfile'     => $mfstOutput,
    'service'     => 'MpAn',
    'session-dir' => $tmpdir,
    'starttime'   => '2003-01-01T00:00',
...
`ncdiff $ncOutputCorrect $ncOutputGot $tmpdir/diff.nc`;
`ncwa -a lat,lon $tmpdir/diff.nc $tmpdir/aavg.nc`;
my $aavg = `ncks -s "%f" -HC -v $var $tmpdir/aavg.nc | head -1`;
chomp $aavg;
ok( abs($aavg) < 1e-10, "Output data is close enough to expected data" );
