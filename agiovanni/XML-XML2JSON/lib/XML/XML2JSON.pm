...
use v5.10.1;   # not really needed, set by Ed Seiler to stop version complaint
use Carp;
use XML::LibXML;
use Tie::IxHash;
...
By default it will use the first one it finds, in the following order: JSON::XS, JSON
...
sub _init {
    ...
    warn "loaded module: $Self->{_loaded_module}" if $Self->{debug};

    # names
    $Self->{attribute_prefix}
        = defined $Args{attribute_prefix} ? $Args{attribute_prefix} : '@';
    $Self->{content_key}
        = defined $Args{content_key} ? $Args{content_key} : '$t';
    ...
    if ( $Args{private_attributes} ) {
        foreach my $private_attribute ( @{ $Args{private_attributes} } ) {
            ...
            $Self->{private_attributes}
                ->{ $Self->{attribute_prefix} . $private_attribute } = 1;
        }
    }
...
sub obj2json {
    ...
    carp "Converting obj to json using $Self->{_loaded_module}"
        if $Self->{debug};
...
# I don't think I like this, but it's more predictable than array folding
        if ( $Self->{force_array} ) {
            warn "Forcing \"$NodeName\" element into an array"
                if $Self->{debug};
...
# check to see if a sibling element of this node name has already been added to the current object block
            if ( exists $CurrentObj->{$NodeName} ) {
                ...
                if ( $NodeType eq 'HASH' ) {
                  ...
                  # so take the sibling element and wrap it inside of an array
                    ...
                    warn "Found the second \"$NodeName\" child element."
                        . " Now wrapping it into an arrayref"
                        if $Self->{debug};
         ...
         # the attribute is going to get overwritten :(
                    ...
                    warn
                        "The \"$NodeName\" attribute conflicts with a child element of the same name."
                ...
                warn "Adding the \"$NodeName\" child element to the array"
                    if $Self->{debug};
                push @{ $CurrentObj->{$NodeName} }, $ElementHash;
            }
...
# prefix the attribute name so that the name cannot conflict with child element names
                warn
                    "Adding attribute to the \"$NodeName\" element: $AttrName"
                    if $Self->{debug};
        ...
        if (@Namespaces) {
            foreach my $Ns (@Namespaces) {
                ...
                my $URI    = $Ns->declaredURI;
                $Prefix = ":$Prefix" if $Prefix;
                $ElementHash->{ $Self->{attribute_prefix} . 'xmlns'
                        . $Prefix } = $URI;
                warn "xmlns$Prefix=\"$URI\"" if $Self->{debug};
            }
        }
                ...
                if ( $Self->{empty_elements}->{$Key} ) {
                    ...
                    foreach my $Attribute (@Attributes) {
                        unless ( ref( $Obj->{$Key}->{$Attribute} ) ) {
                            warn
                                "Deleting attribute from \"$Key\" element: $Attribute"
                                if $Self->{debug};
...
sub json2obj {
    ...
    carp "Converting json to obj using $Self->{_loaded_module}"
        if $Self->{debug};
...
sub obj2dom {
    ...
    my $Version 
        = $Obj->{ $Self->{attribute_prefix} . 'version' }
        || $Obj->{'version'}
        || '1.0';
    my $Encoding 
        = $Obj->{ $Self->{attribute_prefix} . 'encoding' }
        || $Obj->{'encoding'}
        || 'UTF-8';
    ...
    foreach my $Key ( keys %$Obj ) {
        ...
        warn
            "Value ref type for $Key is: $RefType (value seems to be $Obj->{$Key})"
            if $Self->{debug};
