...
sub check_module {
    ...
    if ($@) {
        ...
        eval {
            ...
            # check attribute
            die "$Module: attribute test failed"
                unless $JSON =~ /["']\@attr1["']\s*:\s*["']test["']/;

            # check element text
            die "$Module: text test failed"
                unless $JSON =~ /["']\$t["']\s*:\s*["']some test text["']/;

            my $Object = $XML2JSON->json2obj($JSON);

            # make sure arrays work
            die "did not create array of elements correctly"
                unless $Object->{test}->{array}->{item}->[2]->{'@index'} eq
                    '2';

            # test sanitize
            die "$Module: private element was not removed"
                if $Object->{test}->{private};
            die "$Module: empty element is not empty"
                if grep /^\@/, keys %{ $Object->{test}->{empty} };
            die "$Module: empty element destroyed child"
                unless $Object->{test}->{empty}->{inner};
            die "$Module: private attribute \@foo was not removed"
                if $Object->{test}->{censored}->{'@foo'};
            die "$Module: private attribute \@hidden was not removed"
                if $Object->{test}->{array}->{'@hidden'};
            ...
            my $item2 = $dom->documentElement->findvalue(
                'array/item[@index=2]/@index');
