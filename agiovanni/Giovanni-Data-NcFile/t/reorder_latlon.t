#$Id: reorder_latlon.t,v 1.2 2014/05/07 17:25:38 rstrub Exp $
...
$coords =~ s/^\s+//g;
$coords =~ s/\s+$//g;
my $ncfile = Giovanni::Data::NcFile->new( NCFILE => $outfile, verbose => 1 );
$ncfile->reorder_latlon_dimensions();
...
is( $snowafter,   "float snow(nlayer, nlat, nlon) ;" );
is( $coords,      qq(snow:coordinates = "nlayer nlon nlat" ;) );
is( $coordsafter, qq(snow:coordinates = "nlayer nlat nlon" ;) );
exit(0);
