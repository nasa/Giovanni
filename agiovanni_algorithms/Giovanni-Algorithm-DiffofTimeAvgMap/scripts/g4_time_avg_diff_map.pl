...
#--units unitsA,unitsB,unitsCfgFile
...
use vars qw($opt_b $opt_x $opt_y $opt_s $opt_e $opt_f $opt_o $opt_u);
getopts('b:x:y:s:e:f:o:u:z:');
...
foreach my $filelist (@infiles) {
    ...
    my $rc
        = Giovanni::Algorithm::TimeAveragerNco::compute_time_average(
        $filelist, $outputfile, \@coords, $mainvar, $zvar, $zinfo,
        $time_subset );
...
my @outfiles = @$regriddedList;
@outfiles = convert_units( $opt_u, join( ',', @vars ), @outfiles ) if $opt_u;

# End regridder stuff:

# Rename variables to be the same so we can use ncdiff
run_command( '', 'ncrename', '-v', "$vars[0],time_matched_difference",
    '-O', '-o', $outfiles[0], $outfiles[0] );
run_command( '', 'ncrename', '-v', "$vars[1],time_matched_difference",
    '-O', '-o', $outfiles[1], $outfiles[1] );

# Convert units if necessary
run_command( 'Differencing the two files',
    'ncdiff', '-O', $outfiles[1], $outfiles[0], $opt_o );

# remove time as dimension for variable
# ncwa -O -o no_time.nc -v time_matched_difference -a time sort.out.nc
# ncks -x -v time no_time.nc no_time_var.nc
run_command(
    "Removing time dimension", 'ncwa', '-O',                      "-o",
    $opt_o,                    '-v',   'time_matched_difference', '-a',
    'time',                    $opt_o
);

# Remove time variable
run_command( "Removing time variable and bounds",
    'ncks', '-O', '-x', "-v", 'time,time_bnds', $opt_o, $opt_o );
...
sub convert_units {
    ...
    foreach my $converter (@converters) {
        ...
        # Scalar value of 0 means don't convert
        if ( $converter == 0 ) {
            push @outfiles, $infile;
        }
        else {
            my $dir = dirname($infile) || '.';
            my $newfile = "$dir/uc." . basename($infile);
            $converter->ncConvert(
                sourceFile      => $infile,
...
sub getFirstFile {
    ...
    if ( open FILE, $file ) {
        ...
        warn "ERROR Could not get sample file from $file\n";
    }
}
...
sub add_lat_lon {
    ...
    run_command( 'add lat dimension',
        'ncecat', '-O', '-u', 'lat', '-o', $file, $file );
    run_command( 'add lon dimension',
        'ncecat', '-O', '-u', 'lon', '-o', $file, $file );
    my $ncap2 = sprintf( 'lat[$lat]=%f; lon[$lon]=%f;', $bbox[1], $bbox[0] );
    run_command( 'add lat-lon vars',
        'ncap2', '-s', $ncap2, '-O', '-o', $file, $file );
    run_command( 'reorder dimensions',
        'ncpdq', '-a', 'time,lat,lon', '-O', '-o', $file, $file );
...
sub split_input_file_to_arrays {
    ...
    if ( open FILE, "$file" ) {
        ...
        warn
            "ERROR Could not create arrays for input into regridder from file:<$file>\n";
        return;
    }
